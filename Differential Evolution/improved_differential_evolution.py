...
"""
@Target :   
@File   :   improved_differential_evolution.py    
@author :   yinzikang

@Modify Time      @Author    @Version    @Description
------------      -------    --------    -----------
4/18/22 3:16 PM   yinzikang      1.0         None
"""
import numpy as np
...
def mutation(population_number, population_origin, mutation_operator_ratio, generation_index, best, generation_number):
    index_table = range(population_number)
    population_mutation = np.zeros_like(population_origin)
    adapted_ratio = mutation_operator_ratio * 2 ** np.exp(
        1 - float(generation_number) / (generation_number + 1 - generation_index))
    for i in range(population_number):
        mutation_index = np.random.choice(np.delete(index_table, i), 5, replace=False)
        # basic
        # population_mutation[i] = population_origin[mutation_index[0]] + adapted_ratio * \
        #                          (population_origin[mutation_index[1]] - population_origin[mutation_index[2]])
        # more
        # population_mutation[i] = population_origin[mutation_index[0]] + adapted_ratio / 2 * \
        #                          (population_origin[mutation_index[1]] - population_origin[mutation_index[2]] +
        #                           population_origin[mutation_index[3]] - population_origin[mutation_index[4]])
        # import best and adaptive ratio
        population_mutation[i] = population_origin[mutation_index[0]] + adapted_ratio / 2 * \
                                 (population_origin[mutation_index[1]] - population_origin[mutation_index[2]] +
                                  population_origin[best] - population_origin[i])
        # import noise
        # population_mutation[i] = population_origin[mutation_index[0]] + adapted_ratio / 2 * \
        #                          (population_origin[mutation_index[1]] - population_origin[mutation_index[2]] +
        #                           population_origin[best] - population_origin[i]) - \
        #                          0.1 * np.random.normal(size=population_mutation[i].shape)
...
def crossover(population_number, individual_dimension, population_origin, population_mutation,
    ...
    for i in range(population_number):
        ...
        # with uniform
        population_crossover[i] = np.where(crossover_table < crossover_operator_ratio + 0.5 * np.random.uniform(),
                                           population_origin[i], population_mutation[i])
        # with noise
        # population_crossover[i] = np.where(crossover_table < crossover_operator_ratio + np.random.normal(),
        #                                    population_origin[i], population_mutation[i])
